...
//  aoc24Tests
//
//  Created by Pedro Neto on 02/12/23.
//
...
final class day2: XCTestCase {
    func test1() {
        ...
        for line in input {
            var currGame = generateGame(line)
            if !currGame.isImpossible {
                resultPt1 += currGame.id
            }
         
            resultPt2 += currGame.maxRed * currGame.maxBlue * currGame.maxGreen
        }
    ...
    func generateGame(_ game: String) -> Game {
        let infos = game.components(separatedBy: " ")
        let id = Int(infos[1].dropLast())!
        
        var draw = Draw()
        var draws = [Draw]()
        
        for i in 3..<infos.count {
             
            if infos[i].hasPrefix("red") {
                draw.red = Int(infos[i-1])!
            } else if infos[i].hasPrefix("green") {
                draw.green = Int(infos[i-1])!
            } else if infos[i].hasPrefix("blue") {
                draw.blue = Int(infos[i-1])!
            }
            
            if infos[i].last == ";" || i == (infos.count - 1) {
                draws.append(draw)
                draw = Draw()
            }
        }
