...
//  aoc24Tests
//
//  Created by Pedro Neto on 03/12/23.
//
...
final class day3: XCTestCase {
    func test1() {
        ...
        for i in 0..<input.count {
            for j in 0..<input[i].count {
                ...
                if (size > 0 && j < (input[i].count - 1) && !input[i][j+1].isNumber) || (j == input[i].count - 1) {
                    // check line above
                    if (i - 1 >= 0) {
                        if (j - size) >= 0 && input[i-1][j-size].isSpecial {
                            partNumbers.append(Int(number)!)
                        }
                        
                        for k in 0..<size {
                            if input[i-1][j-k].isSpecial {
                                partNumbers.append(Int(number)!)
                            }
                        }
                        
                        if j < (input[i-1].count - 1) && input[i-1][j+1].isSpecial {
                            partNumbers.append(Int(number)!)
                        }
                    }
                    
                    // check before
                    if (j - size) >= 0 && input[i][j-size].isSpecial { partNumbers.append(Int(number)!) }
                    
                    // check after
                    if j < (input[i].count - 1) && input[i][j+1].isSpecial { partNumbers.append(Int(number)!) }
    ...
    func test2() {
        let input = gameInputDemo.components(separatedBy: "\n").map{ Array($0) }
        var partNumbers = [Int]()
        
        for i in 0..<input.count {
            for j in 0..<input[i].count {
                let curr = input[i][j]
                guard curr == "*" else { continue }
                var numbers = [Int]()
                // check top
                if i > 0 {
                ...
                // check left
                if j > 0 && input[i][j-1].isNumber {
                    var number = ""
                    for k in (0..<j).reversed() {
                        if input[i][k].isNumber {
                            number.insert(input[i][k], at: number.startIndex)
                        }
                        if input[i][k] == "." || k == 0 {
                ...
                // check right
                if j < (input[i].count - 1) && input[i][j+1].isNumber {
                    var number = ""
                    for k in j..<input[i].count {
                        if input[i][k].isNumber {
                            number.append(input[i][k])
                        }
                        if input[i][k] == "." || k == 0 {
                            numbers.append(Int(number)!)
                            number = ""
                ...
                partNumbers.append(numbersTotal)
            }
        }
        print(partNumbers)
    }
